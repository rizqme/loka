...
- **Port forwarding** -- expose a VM port on your machine with `--port 8080:5000` at creation or `loka session port-forward` on-demand. See [Sessions](/sessions) for details.
