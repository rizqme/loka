...
### 6. Run a command with port forwarding

Create a session with a forwarded port and start a web server inside the VM. The server is reachable at `localhost:8080` on your machine.
...
client.run_and_wait(session.id, "python3 -m http.server 5000")
```
...
await loka.runCommand(session.id, 'python3 -m http.server 5000');
```
